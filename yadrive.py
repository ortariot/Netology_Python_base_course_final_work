...
class YaDrive:
    ...
    def upload_from_drive(self, filename, path_to_cloud=None,
                          path_to_folder=''):
        if '\\' in filename:
        ...
        link = requests.get(url, headers=headers,
                            params=params).json().get('href', None)
        if link is None:
            return 'Unable to get upload link'
        else:
            response = requests.put(link, data=open(path_to_folder + filename,
                                    'rb'))
    ...
    def upload_from_url(self, upload_url, filename, path_to_cloud=None):
        ...
        params = {'path': path_to_cloud + filename,
                  'overwrite': 'true',
                  'url': upload_url
                  }
